"""Engine test: base ledger: job/hand blocks, edges, notes, attention, character reject."""
import argparse
import json
...
DESIGN = [{"name": "wooden-chest", "x": 0.5, "y": 0.5}, {"name": "stone-furnace", "x": 3, "y": 1}]
...
def main():
    ...
    shutil.copy2(HERE / "test_ledger_runtime.lua", MOD / "test_ledger_runtime.lua")
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        f.write('\nrequire("test_ledger_runtime")(HANDLERS, bridge_state, '
                + json.dumps(encode_blueprint(DESIGN)) + ")\n")
    save = SANDBOX / "saves" / "ledger-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "ledger-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "ledger-benchmark.log"
    with log.open("w", encoding="utf-8") as output:
        run = subprocess.run([
            str(EXE), "--config", str(SANDBOX / "config.ini"),
            "--mod-directory", str(SANDBOX / "mods"),
            "--benchmark", str(save), "--benchmark-ticks", "7200",
