...
4. **Check.** Call `observe(view="situation")`. An `ok` reply with your `treasury` means the
   game, the mod and the server are connected. Its `build` field compares the loaded mod's
   `BRIDGE_BUILD` with this repo's `factorio-ai-bridge_0.1.0/core.lua`: `stale: true` means the
   game still runs an old copy of the mod (copy it over and restart the game).
