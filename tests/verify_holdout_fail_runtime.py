"""Engine test of the executor holdout-FAIL path (coal pattern, feed dropped + thin primer) in a save copy."""
import argparse
import json
...
def main():
    ...
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        f.write('\nrequire("test_holdout_fail_runtime")(HANDLERS, bridge_state, '
                + json.dumps(pattern["blueprint_string"]) + ", "
                + json.dumps(json.dumps(pattern["contract"])) + ")\n")
    save = SANDBOX / "saves" / "executor-fail-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "executor-fail-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "executor-fail-benchmark.log"
