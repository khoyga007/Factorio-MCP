...
## Ledger (base memory, build `2026-09-20-flow-truth`)
...
- `observe(view="ledger")` → `blocks` [{id, name, role, feeds, eats, notes, status, box [x1,y1,x2,y2], n {entity: planned count}, missing?, error?, pattern_id, tick, flow?}] + `edges` [{from, to, item?, declared?, measured?, uncounted?, missing_block?}].
