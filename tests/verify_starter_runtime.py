"""Run the production starter planner in a separate Factorio benchmark save."""
import argparse
import json
...
def main():
    ...
    shutil.copy2(HERE / "test_starter_runtime.lua", MOD / "test_starter_runtime.lua")
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        f.write('\nrequire("test_starter_runtime")(HANDLERS, bridge_state)\n')
    save = SANDBOX / "saves" / "starter-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "starter-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "starter-benchmark.log"
    ...
    for name in ("iron", "copper"):
        if name in data and not (SANDBOX / "script-output" / f'{data[name]["artifact"]}.blueprint.txt').exists():
            data["ok"] = False
            data["error"] = f"missing-{name}-blueprint"
    print(json.dumps({"ok": data["ok"], "error": data.get("error"),
                      "checks": len(data["checks"]), "iron": data.get("iron"),
                      "copper": data.get("copper"), "artifact": str(result)}, indent=2))
