"""Run the coal cell in a copied Factorio save, never in the player's live save."""
import argparse
import json
...
def main():
    ...
    result = SANDBOX / "script-output" / "coal-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "coal-benchmark.log"
    with log.open("w", encoding="utf-8") as output:
        run = subprocess.run([
            str(EXE), "--config", str(SANDBOX / "config.ini"),
            "--mod-directory", str(SANDBOX / "mods"),
            "--benchmark", str(save), "--benchmark-ticks", "2400",
    ...
    if data.get("job_id") and not (SANDBOX / "script-output" / "coal"
        / f'{data["job_id"]}.blueprint.txt').exists():
        data["ok"] = False
        data["error"] = "missing-coal-blueprint"
