"""Engine test: agent-authored design (encode_blueprint) built + audited by the executor in a save copy."""
import argparse
import json
...
def main():
    ...
    parser.add_argument("--design", type=Path, default=HERE / "designs" / "coal-drill-chest.json")
    ...
    shutil.copy2(HERE / "test_design_runtime.lua", MOD / "test_design_runtime.lua")
    spec = json.loads(args.design.read_text(encoding="utf-8"))
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        f.write('\nrequire("test_design_runtime")(HANDLERS, bridge_state, '
                + json.dumps(encode_blueprint(spec["design"])) + ", "
                + json.dumps(json.dumps(spec["contract"])) + ")\n")
    save = SANDBOX / "saves" / "design-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "design-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "design-benchmark.log"
