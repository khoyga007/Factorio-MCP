...
10. **Split big jobs, use exact sites** (see factorio-build-design-anchor-trap memory).
...
13. **Restart discipline:** `luac -p` → copy mod → `cmp` → ask maintainer to restart Factorio (Lua) and MCP (Python). Lua fix is dead until restart; note it as pending.
...
16. **Spectacle moments need the human watching.** Rocket 1 launched while maintainer wasn't looking — had to build rocket 2. Before an irreversible visible milestone, say where to look and wait for "go".
17. **Report mistakes on sight, pivot fast.** The purple-science error was admitted and pivoted in one message; maintainer approved immediately.
