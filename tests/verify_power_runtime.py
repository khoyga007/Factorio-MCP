"""Engine test: live-grid power pre-check, per-entity occupancy, resume of a built job."""
import argparse
import json
...
# lab + a pole beside it + a pole 7 tiles out: the far pole stretches the bounding box
# well past the tiles the design actually occupies.
MAIN = [{"name": "lab", "x": 1.5, "y": 1.5},
        {"name": "small-electric-pole", "x": 3.5, "y": 1.5},
        {"name": "small-electric-pole", "x": 10.5, "y": 1.5}]
CHEST = [{"name": "wooden-chest", "x": 0.5, "y": 0.5}]
...
def main():
    ...
    sync_mod(MOD)
    shutil.copy2(HERE / "test_power_runtime.lua", MOD / "test_power_runtime.lua")
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        # A Lua table literal: JSON object syntax is not Lua.
        f.write('\nrequire("test_power_runtime")(HANDLERS, bridge_state, {main='
                + json.dumps(encode_blueprint(MAIN)) + ", chest="
                + json.dumps(encode_blueprint(CHEST)) + "})\n")
    save = SANDBOX / "saves" / "power-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "power-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "power-benchmark.log"
