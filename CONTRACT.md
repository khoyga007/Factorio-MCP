...
`achieve(goal="build_design" | "reuse_blueprint", ...)` → Lua `executor.lua` (action `blueprint_run`). `build_design` sends rows the agent designed; `reuse_blueprint` sends a saved pattern. Agent designs + declares; executor checks, gathers real items, builds, primes, audits. No per-pattern code. Contract omitted → pattern's saved `contract` in `blueprints/catalog/<id>.json` → else `{}` (search around x,y, no primer, no audit).

Job id `exec-N`; read with `report(job_id)` (action `blueprint_job`). Receipt + audit + blueprint in script-output `executor/exec-N.*`. Up to 8 jobs may be live at once; repeating the same call returns that job instead of starting a second (§Site, several live jobs).
...
- [Glossary](#glossary)
- [Executor rules (cheat sheet)](#executor-rules-cheat-sheet)
- [Schema](#schema)
- [Site](#site)
- [Materials](#materials)
- [Build](#build)
- [Poles and power](#poles-and-power)
- [Ghost build (build.mode="ghost")](#ghost-build-buildmodeghost)
- [Holdout audit](#holdout-audit)
- [Feeds](#feeds)
- [Resume](#resume)
- [Agent-authored design (build_design)](#agent-authored-design-builddesign)
- [Rate plan, production cells and chains](#rate-plan-production-cells-and-chains)
- [Ground clearing: cliffs and water](#ground-clearing-cliffs-and-water)
- [Research, recipes and hand work](#research-recipes-and-hand-work)
- [Field actions: water sites and recall](#field-actions-water-sites-and-recall)
- [Ledger (base memory)](#ledger-base-memory)
- [Reference imports (community blueprints, catalog_add.py)](#reference-imports-community-blueprints-catalogaddpy)
...
- Anchor = top-left TILE EDGE of the rotated blueprint footprint (`min(entity.x - tile_width/2)`), NOT an entity centre.
  ...
  - `capture(area)` and `build_ghosts(area)` therefore return `site {x,y}` — the exact anchor, computed in Lua from `create_blueprint`'s blueprint-index -> source-entity mapping, so it covers exactly the entities the blueprint holds, ghosts included. Hand that back to `reuse_blueprint` with `site.mode="exact"`; never re-derive it.
...
- Engine PASS tests/verify_ghostbuild_runtime.py: three mixed footprints (drill 3x3 at `.5`, furnace 2x2 at integer, pole 1x1 at `.5`) export anchor `(-29,-11)`, the anchor round-trips through `blueprint_run`, and the planned centres come back exactly where the ghosts stand.
- Engine PASS tests/verify_inserter_runtime.py: lab 1 tile off → drop gap + shift [-1,0] (also under rotation 4), fixed → planned, pickup from existing belt counts, engine pickup/drop positions match.
...
Re-check site + stock right before build. Then trees/rocks inside any entity footprint (tile box, not margin) `mine`d → products to bag, receipt each, `cleared` = count on the job (once). `dry_run`/job `site.clear` = count to clear. Nature outside footprints untouched (trees absorb pollution). Exact landing of native `build_blueprint` found by a ghost probe (ghosts built then destroyed, zero items), then `blueprint_import` direct mode at the corrected position with rotation. Every entity must then sit at its planned position/direction or `import-geometry-mismatch`. Primer inserted from treasury (receipt per insert). Primer is the LAST executor mutation.

### Evidence

- Engine PASS 2026-09-19 tests/verify_clear_runtime.py: chest+furnace in forest + big-rock → 6 cleared, footprints empty, tree outside kept, wood+stone in bag; cliff under site with no explosives and the recipe locked → `cliff` reject, cliff intact; with explosives in the bag the same site quotes `cliff-explosives` in `materials`.
...
- Since build `2026-09-20-ghost-build`.
...
- Design origin: PORTING §5, stricter than FLE.
...
- Since build `2026-09-20-resume-power`.
...
## Rate plan, production cells and chains
...
- `build_design` row `{chain:item@N/min, x, y}` (`chain.py`): plan → one producer cell PER consumer (no splitters), stacked in a column (gap 4), post-order (leaves on top). Each internal edge = routed belt child `out` → parent `in` via Lua `route` with `avoid` (cell boxes + reserved port tiles) + `planned_belts` (cell + earlier route belts, so no route sideloads into them).
  - Chain can't make it (an item whose recipe a cell refuses, or the pre-merged belt of a 3-4 ingredient cell) → `ports.external` {items, x, y, dir, cell, per_min}: agent feeds those. Furnace cells chain like any other (live exec-19: iron-plate and copper-plate stone-furnace cells). `output` = tile east of root out belt.
  - Fluids: a fluid recipe becomes a fluid cell and its SOLID inputs are chained, but fluids are never chained and never appear in `ports.external`. Each fluid cell's pipe ports (`fluid_in`/`fluid_out`, `{kind:"pipe",x,y,fluid}`) are in its `ports.cells[]` entry: pipe them yourself. A ROOT that makes only a fluid outputs on its pipe bus: `output = {item, kind:"pipe", x, y}` (east end of the bus).
  - Feeding a chain's external ports by hand (prefer `feed:true` / `{mine:...}` rows, below): feeds are agent-made: electric drills (search + resource rule exclusive) → `observe(route)` → `{route:id}` rows. External port tile = EMPTY tile west of cell belt; agent puts a belt there (dir E) and side-loads ore from N (north lane) + coal from S (south lane). Two side inputs, no rear → belt stays straight (checked by `lanes`: N iron-ore, S coal). Build ore routes first, coal routes after, port belts LAST (one side input alone = curve → mixed lanes).
...
- `build_design` row `{mine:ore, to:[x,y], lane?:"N"|"S", per_min?, drill?, pole?}` and chain row flag `feed:true` (24/09, gap "a built cell never wires itself to mines"). World rows only: refused with an achieve `x,y` (`mine-needs-world-rows-no-x-y` / `feed-needs-world-rows-no-x-y`).
  ...
  - `feed:true` on a chain mines every external item whose `spec` entity is a `resource`; 2-item ports get lanes N/S, 1-item ports a rear feed. Non-ore externals (plates into a cable cell) stay external. Reply `ports[].fed` = [{ore, to, lane, drills, box, route_belts}].
  - `lane_joins` (Lua) now counts two opposite side inputs with no rear input as `sideload`, not `curve`: the engine keeps such a belt straight.
...
- LIVE BUILD 23/09 new save (tick ~50k, from vanilla start): electronic-circuit@60 at (-62,28), exec-16 ghost mode → verified; circuits on output belt row 66.5.
- Live dry-run 24/09 on exec-19's two ports (iron+coal at (-40.5,60.5), copper+coal at (-40.5,70.5)): 4 drills on 4 patches, 214 belts + 10 undergrounds, 6 poles, `planned`. Real build not run: the bag had 0 drills and 5 iron plates.
...
- Handlers: `build.lua` `handle_research`, `handle_set_recipe`; `items.lua` hand handlers.
...
## Field actions: water sites and recall
...
- Handlers: `field.lua`, written for live play.
...
- Throughput (`flow`, build `2026-09-20-flow-truth`): each block carries `{ticks, samples, made {item: per minute}, active {entity: MEAN percent of that entity's samples spent working}, idle {entity: {reason: percent}}, counted}` from its last CLOSED window (default 3600 ticks; `storage.ledger_flow_window` overrides, tests use 300). Absent until the first window closes.
  - `idle` (build `2026-09-24-idle`): machines only (assembler, furnace, silo, drill, lab), ≥1%. Reasons: `starved` (no/short ingredients or fluid, missing packs → fix UPSTREAM), `blocked` (output full / no space in destination → fix DOWNSTREAM), `power`, `fuel`, `depleted`, `recipe`, `research`, `other`. An edge whose `measured < declared/2` carries `why` = the producer's worst idle reason. Engine PASS tests/verify_ledger_runtime.py (43 checks, 24/09): fuelled furnace with no ore `starved 100`, furnace with a full result slot `blocked 100` (its first window reads 50: the craft in hand finishes first); the old furnace edge read `why: fuel` (17% out of coal).
...
- Since build `2026-09-21-ledger-zoom`.
