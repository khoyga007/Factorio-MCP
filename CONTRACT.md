...
- `build_design` row `{chain:item@N/min, x, y}` (`chain.py`): plan → one producer cell PER consumer (no splitters), stacked in a column (gap 4), post-order (leaves on top). Each internal edge = routed belt child `out` → parent `in` via Lua `route` with `avoid` (cell boxes + reserved port tiles) + `planned_belts` (cell + earlier route belts, so no route sideloads into them).
  ...
  - Gaps hit live (open): `direct` mode refuses a whole chain (`direct-blueprint-too-large`) → use ghost mode. `power:true` → `power-route:from-blocked` when a pole sits right beside the bbox; chain's own poles reached it anyway, but cells 8 apart (circuit ↔ cable) needed a hand pole (-60.5,56.5). Parallel jobs race for the same bag item (`materials-changed:wood`); prepare's own crafts eat each other (steam-engine recipe ate the pipe → `materials-changed:pipe`) → pre-`craft` then build. Boiler `connect` water check fires before water flows → false `infra-missing:water`, `report(resume)` clears it.
