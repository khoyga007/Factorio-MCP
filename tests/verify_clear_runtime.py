...
def main():
    ...
    shutil.copy2(HERE / "test_clear_runtime.lua", MOD / "test_clear_runtime.lua")
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        f.write('\nrequire("test_clear_runtime")(HANDLERS, bridge_state, '
                + json.dumps(encode_blueprint(DESIGN)) + ")\n")
    save = SANDBOX / "saves" / "clear-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "clear-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "clear-benchmark.log"
