"""Engine test: research trigger list + backlog with an empty lab in a save copy."""
import argparse
import json
...
def main():
    ...
    sync_mod(MOD)
    shutil.copy2(HERE / "test_research_runtime.lua", MOD / "test_research_runtime.lua")
    with (MOD / "control.lua").open("a", encoding="utf-8") as f:
        f.write('\nrequire("test_research_runtime")(HANDLERS, bridge_state)\n')
    save = SANDBOX / "saves" / "research-player.zip"
    shutil.copy2(args.player_save, save)
    result = SANDBOX / "script-output" / "research-check.json"
    result.unlink(missing_ok=True)
    log = SANDBOX / "research-benchmark.log"
